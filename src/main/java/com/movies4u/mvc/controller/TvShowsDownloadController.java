...
import com.movies4u.mvc.services.TvShowsDownloadServices;

@RestController
@RequestMapping("/tv/download")
@CrossOrigin
public class TvShowsDownloadController {

	@Autowired
	private TvShowsDownloadServices downloadServices;
	...
	public ResponseEntity<?> getDownloadByMoviesID(@PathVariable Long id){
		...
		List<TvShowsDownload> Downloadlist = downloadServices.getDownloadByMoviesID(id);
	...
	@PostMapping("{id}")
	public ResponseEntity<?> addDownloadByMoviesID(@PathVariable("id") Long id,@RequestBody TvShowsDownload download){
		if(id<=0) return new ResponseEntity<String>("download id must be greater than 0",HttpStatus.BAD_REQUEST);
		return new ResponseEntity<TvShowsDownload>(downloadServices.addDownloadByMoviesID(id, download),HttpStatus.CREATED);
	}
	
	@PutMapping("{id}")
	public ResponseEntity<?> updateDownloadByid(@PathVariable("id") Long id,@RequestBody TvShowsDownload download){
		if(id<=0) return new ResponseEntity<String>("download id must be greater than 0",HttpStatus.BAD_REQUEST);
